...
(function() {
    ...
    function createPopupContent(point, isAvailable) {
        ...
        if (!isAvailable && point.collectorInfo) {
            ...
            // Добавляем селфи если есть - теперь передаем информацию о сборщике
            if (point.collectorInfo.selfie) {
                // Экранируем JSON для безопасной передачи в onclick
                const collectorInfoJson = JSON.stringify(point.collectorInfo).replace(/"/g, '&quot;');
                ...
                popupContent += `<img src="${point.collectorInfo.selfie}" 
                                  ...
                                  onclick="showFullImage('${point.collectorInfo.selfie}', '${point.name}', JSON.parse('${collectorInfoJson}'.replace(/&quot;/g, '\\\"')))" 
