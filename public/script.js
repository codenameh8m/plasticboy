...
let abortController = null; // Для отмены запросов
...
function safeNotifyLoader() {
    try {
        if (typeof window.PlasticBoyLoader !== 'undefined' && 
            ...
            debugLog('Уведомили загрузчик о готовности точек');
        } else {
            debugLog('Загрузчик не найден или не готов');
        }
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        ...
        // Инициализируем карту только после загрузки всех скриптов
        if (typeof L !== 'undefined') {
            debugLog('Leaflet готов, инициализируем карту');
            initMap();
        } else {
            debugLog('Ждем загрузки Leaflet...');
...
function addDebugMarkerStyles() {
    if (document.getElementById('debug-marker-styles')) {
...
function initMap() {
    ...
    try {
        ...
        if (!mapElement) {
            debugLog('ОШИБКА: элемент #map не найден');
            return;
        }
...
function addTestMarker() {
    try {
        ...
        // Убираем тестовый маркер через 15 секунд
        setTimeout(() => {
            try {
                map.removeLayer(testMarker);
                debugLog('Тестовый маркер удален');
            } catch (e) {
                debugLog('Ошибка при удалении тестового маркера:', e);
            }
        }, 15000);
...
async function loadPointsWithDebug() {
    ...
    try {
        ...
        // Безопасно уведомляем систему загрузки
        safeNotifyLoader();
...
function createTestPoints() {
    ...
    const testPoints = [
        ...
        {
            id: 'test3',
            name: 'Тестовая точка 3',
            coordinates: { lat: 43.2180, lng: 76.8480 },
            status: 'available'
        }
    ];
    ...
    showDebugNotification('Добавлены тестовые точки на карту', 'info');
}
...
async function updateMapWithDebug(points) {
    ...
    try {
        ...
        if (points.length === 0) {
            ...
            showDebugNotification('Нет точек для отображения', 'warning');
            return;
        }
        ...
        if (errorCount > 0) {
            showDebugNotification(`Ошибок при добавлении маркеров: ${errorCount}`, 'error');
        }
